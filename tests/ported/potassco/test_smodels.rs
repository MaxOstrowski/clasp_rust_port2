...
use rust_clasp::potassco::basic_types::{
    AbstractProgram, Atom, DomModifier, HeadType, Lit, LitSpan, TruthValue, Weight, WeightLit,
    WeightLitSpan,
};
use rust_clasp::potassco::error::Error;
use rust_clasp::potassco::smodels::{
    SmodelsInput, SmodelsOptions, SmodelsOutput, SmodelsType, match_dom_heu_pred, match_edge_pred,
    read_smodels,
};
use rust_clasp::potassco_check_pre;

use crate::test_common::{Heuristic, ReadObserver, Vec, to_cond};
...
fn smodels_reader_parses_core_directives() {
    ...
    assert_eq!(
        observer.rules[&SmodelsType::Basic][0],
        vec![2, -4, -2, 3, 5]
    );
    assert_eq!(
        observer.rules[&SmodelsType::Choice][0],
        vec![2, 3, 4, -5, 6]
    );
    assert_eq!(
    ...
    assert_eq!(
        observer.rules[&SmodelsType::Cardinality][0],
        vec![2, 1, -3, 4]
    );
    assert_eq!(
...
fn smodels_input_type_is_constructible_directly() {
    let mut observer = TestObserver::default();
    let mut reader = rust_clasp::potassco::match_basic_types::ProgramReader::new(
        SmodelsInput::new(&mut observer, SmodelsOptions::default()),
    );
    let mut text = String::new();
    finalize(&mut text, &[], "", "1");
    assert_eq!(
        rust_clasp::potassco::match_basic_types::read_program(
